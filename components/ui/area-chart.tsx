...
function resolveWidth(width: IndicatorWidth): number {
  ...
  switch (width) { case "line": return 1; case "thin": return 2; case "medium": return 4; case "thick": return 8; default: return 1; }
}

function TooltipIndicator({ x, height, visible, width = "line", span, columnWidth, colorEdge = chartCssVars.crosshair, colorMid = chartCssVars.crosshair, fadeEdges = true, gradientId = "tooltip-indicator-gradient" }: {
  ...
  useEffect(() => {
    ...
    if (isWaitingForSettlement) {
      frameRef.current = requestAnimationFrame(() => {
        frameRef.current = requestAnimationFrame(() => { setCommittedHeight(bounds.height); committedChildrenStateRef.current = hasChildren; });
      });
    } else {
          ...
          {rows.map((row) => (
            <div className="flex items-center justify-between gap-4" key={`${row.label}-${row.color}`}>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm" style={{ color: chartCssVars.foregroundMuted }}>{row.label}</span>
              </div>
              <span className="font-medium text-sm tabular-nums" style={{ color: chartCssVars.foreground }}>
...
function TooltipBox({ x, y, visible, containerRef, containerWidth, containerHeight, offset = 16, className = "", children, left: leftOverride, top: topOverride, flipped: flippedOverride }: {
  x: number; y: number; visible: boolean; containerRef: RefObject<HTMLDivElement | null>;
  containerWidth: number; containerHeight: number; offset?: number; className?: string;
  children: ReactNode; left?: number | ReturnType<typeof useSpring>; top?: number | ReturnType<typeof useSpring>; flipped?: boolean;
}) {
  ...
  useEffect(() => { if (prevFlipRef.current !== shouldFlipX) { setFlipKey((k) => k + 1); prevFlipRef.current = shouldFlipX; } }, [shouldFlipX]);

  const cfg = { stiffness: 100, damping: 20 };
  const animatedLeft = useSpring(targetX, cfg);
  const animatedTop = useSpring(targetY, cfg);
  ...
  return createPortal(
    <motion.div animate={{ opacity: 1 }} className={cn("pointer-events-none absolute z-50", className)} exit={{ opacity: 0 }} initial={{ opacity: 0 }} ref={tooltipRef} style={{ left: finalLeft, top: finalTop }} transition={{ duration: 0.1 }}>
      <motion.div animate={{ scale: 1, opacity: 1, x: 0 }} className="min-w-[140px] overflow-hidden rounded-lg shadow-lg backdrop-blur-md" initial={{ scale: 0.85, opacity: 0, x: isFlipped ? 20 : -20 }} key={flipKey} style={{ transformOrigin, backgroundColor: chartCssVars.markerBackground, border: `1px solid ${chartCssVars.markerBorder}` }} transition={{ type: "spring", stiffness: 300, damping: 25 }}>
        {children}
      </motion.div>
    </motion.div>,
    container
...
export function ChartTooltip({ showDatePill = true, showCrosshair = true, showDots = true, content, rows: rowsRenderer, children, className = "" }: ChartTooltipProps) {
  ...
  const cfg = { stiffness: 300, damping: 30 };
  const animatedX = useSpring(xWithMargin, cfg);
  ...
  const tooltipContent = (
    <>
      {showCrosshair && (
        <svg aria-hidden className="pointer-events-none absolute inset-0" height="100%" width="100%">
          <g transform={`translate(${margin.left},${margin.top})`}>
            <TooltipIndicator colorEdge={chartCssVars.crosshair} colorMid={chartCssVars.crosshair} columnWidth={columnWidth} fadeEdges height={innerHeight} visible={visible} width="line" x={x} />
          </g>
        </svg>
      )}
      {showDots && visible && !isHorizontal && (
        <svg aria-hidden className="pointer-events-none absolute inset-0" height="100%" width="100%">
          <g transform={`translate(${margin.left},${margin.top})`}>
            {lines.map((line) => (
              <TooltipDot color={line.stroke} key={line.dataKey} strokeColor={chartCssVars.background} visible={visible} x={tooltipData?.xPositions?.[line.dataKey] ?? x} y={tooltipData?.yPositions[line.dataKey] ?? 0} />
            ))}
          </g>
        </svg>
      )}
...
function XAxisLabel({ label, x, crosshairX, isHovering, tickerHalfWidth }: { label: string; x: number; crosshairX: number | null; isHovering: boolean; tickerHalfWidth: number }) {
  ...
  return (
    <div className="absolute" style={{ left: x, bottom: 12, width: 0, display: "flex", justifyContent: "center" }}>
      <motion.span animate={{ opacity }} className="whitespace-nowrap text-xs" initial={{ opacity: 1 }} style={{ color: chartCssVars.label }} transition={{ duration: 0.4, ease: "easeInOut" }}>
        {label}
      </motion.span>
    </div>
  );
...
export function XAxis({ numTicks = 5, tickerHalfWidth = 50 }: XAxisProps) {
  ...
  const labelsToShow = useMemo(() => {
    ...
    return dates.map((date) => ({ date, x: (xScale(date) ?? 0) + margin.left, label: date.toLocaleDateString("pt-BR", { month: "short", day: "numeric" }) }));
  ...
  return createPortal(
    <div className="pointer-events-none absolute inset-0">
      {labelsToShow.map((item) => <XAxisLabel crosshairX={crosshairX} isHovering={isHovering} key={`${item.label}-${item.x}`} label={item.label} tickerHalfWidth={tickerHalfWidth} x={item.x} />)}
    </div>,
    container
  );
}
...
export function YAxis({ numTicks = 5, formatValue }: YAxisProps) {
  ...
  const ticks = useMemo(() => {
    ...
    return Array.from({ length: numTicks }, (_, i) => {
      ...
      return { value, y: (yScale(value) ?? 0) + margin.top, label: formatValue ? formatValue(value) : value >= 1000 ? `${(value / 1000).toFixed(value % 1000 === 0 ? 0 : 1)}k` : value.toLocaleString("pt-BR") };
    });
...
export function Area({ dataKey, fill = chartCssVars.linePrimary, fillOpacity = 0.4, stroke, strokeWidth = 2, curve = curveMonotoneX, animate = true, showLine = true, showHighlight = true, gradientToOpacity = 0, fadeEdges = false }: AreaProps) {
  ...
  const strokeGradientId = useMemo(() => `area-stroke-gradient-${dataKey}-${Math.random().toString(36).slice(2, 9)}`, [dataKey]);
  ...
  const segmentBounds = useMemo(() => {
    ...
    return { startLength: findLengthAtX(startX), segmentLength: findLengthAtX(endX) - findLengthAtX(startX), isActive: true };
    ...
    <>
      ...
      {animate && (
        <defs>
          <clipPath id={`grow-clip-area-${dataKey}`}>
            <rect height={innerHeight + 20} style={{ transition: !isLoaded && clipWidth > 0 ? `width ${animationDuration}ms ${easing}` : "none" }} width={isLoaded ? innerWidth : clipWidth} x={0} y={0} />
          </clipPath>
        </defs>
      )}
      <g clipPath={animate ? `url(#grow-clip-area-${dataKey})` : undefined}>
        <motion.g animate={{ opacity: isHovering && showHighlight ? 0.6 : 1 }} initial={{ opacity: 1 }} transition={{ duration: 0.4, ease: "easeInOut" }}>
          ...
          {showLine && <LinePath curve={curve} data={data} innerRef={pathRef} stroke={`url(#${strokeGradientId})`} strokeLinecap="round" strokeWidth={strokeWidth} x={(d) => xScale(xAccessor(d)) ?? 0} y={getY} />}
        </motion.g>
      </g>
      {showHighlight && showLine && isHovering && isLoaded && pathRef.current && (
        <motion.path animate={{ opacity: 1 }} d={pathRef.current.getAttribute("d") || ""} exit={{ opacity: 0 }} fill="none" initial={{ opacity: 0 }} stroke={resolvedStroke} strokeLinecap="round" strokeWidth={strokeWidth} style={{ strokeDasharray: animatedDasharray, strokeDashoffset: offsetSpring }} transition={{ duration: 0.4, ease: "easeInOut" }} />
      )}
    </>
  );
}
...
export interface AreaChartProps {
  ...
  data: Record<string, unknown>[];
  /** Key in data objects containing the date/time value (string or number) */
  xKey?: string;
  /** Chart margin */
  margin?: Margin;
  /** Animation duration in ms for the grow-in effect */
  animationDuration?: number;
  className?: string;
  children: ReactNode;
}
...
export function AreaChart({
  ...
  // Collect Line configs from <Area> children
  const lines = useMemo<LineConfig[]>(() => {
    const result: LineConfig[] = [];
    Children.forEach(children, (child) => {
      if (isValidElement(child) && (child as ReactElement<AreaProps>).type === Area) {
        const p = (child as ReactElement<AreaProps>).props;
        result.push({
          dataKey: p.dataKey,
  ...
  const xAccessor = useCallback(
    (d: Record<string, unknown>) => new Date(d[xKey] as string | number),
    [xKey]
  );
  ...
  const xScale = useMemo(() => {
    ...
    const [min, max] = extent(dates, (d) => d.getTime()) as [number, number];
  ...
  const yScale = useMemo(() => {
    const allValues = data.flatMap((d) =>
      lines.map((l) => (typeof d[l.dataKey] === "number" ? (d[l.dataKey] as number) : 0))
    );
    const maxVal = Math.max(...allValues, 1);
    return scaleLinear<number>({ domain: [0, maxVal * 1.15], range: [innerHeight, 0], nice: true });
  ...
  // Trigger isLoaded after animation
  useEffect(() => {
    if (width > 0 && height > 0) {
      setIsLoaded(false);
      const t = setTimeout(() => setIsLoaded(true), animationDuration + 100);
  ...
  const { tooltipData, setTooltipData, selection, clearSelection, interactionHandlers, interactionStyle } = useChartInteraction({
    xScale, yScale, data, lines, margin, xAccessor,
    bisectDate: bisectDate as (data: Record<string, unknown>[], date: Date, lo: number) => number,
    canInteract: true,
  });

  const contextValue: ChartContextValue = {
    data, xScale, yScale, width, height, innerWidth, innerHeight, margin,
    columnWidth, tooltipData, setTooltipData, containerRef, lines, isLoaded,
    animationDuration, xAccessor, dateLabels, selection, clearSelection,
  };

  // Separate SVG children from portal children (tooltip, axes)
  const svgChildren: ReactNode[] = [];
  const portalChildren: ReactNode[] = [];
  Children.forEach(children, (child) => {
    if (isValidElement(child)) {
      const type = (child as ReactElement).type;
      if (type === ChartTooltip || type === XAxis || type === YAxis) {
  ...
  return (
    <ChartProvider value={contextValue}>
      <div ref={containerRef} className={cn("relative w-full h-full", className)} style={interactionStyle}>
        {width > 0 && height > 0 && (
          <svg width={width} height={height} className="overflow-visible">
            <g transform={`translate(${margin.left},${margin.top})`}>
              <g {...interactionHandlers}>
                <rect width={Math.max(0, innerWidth)} height={Math.max(0, innerHeight)} fill="transparent" />
                {svgChildren}
              </g>
            </g>
          </svg>
